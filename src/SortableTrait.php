...
trait SortableTrait
{
    ...
    /**
     * Moves models array to the last position.
     * @param array $ids  the id of model array
     * @param $arrayMaxOrderColumn   the max order_column of the model array
     * @return $this
     */
    public function moveArrayToEnd(array $ids, $arrayMaxOrderColumn)
    {
        // max order column
        $maxOrder = $this->getHighestOrderNumber();
        ...
        // if max order column is last record, just skip it
        if ($arrayMaxOrderColumn === $maxOrder) {
            return $this;
        }

        $this->buildSortQuery()->where($orderColumnName, '>', $arrayMaxOrderColumn)
            ->decrement($orderColumnName, count($ids));

        // get max order now.
        $maxOrderNow = $maxOrder - count($ids) + 1;
        $this->setNewOrder($ids, $maxOrderNow);
        return $this;
    }
}
